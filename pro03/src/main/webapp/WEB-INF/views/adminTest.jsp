 	    ...
 	    <section class="page" id="page1">
    		...
    		<div class="page-wrap">
    			...
    			<form>      <!-- 메시지 텍스트 박스 -->      
    				...
    				<input onclick="sendADMessage()" value="Send" type="button" class="sendBtn">    
    			</form>    
    			<br />    <!-- 서버와 메시지를 주고 받는 콘솔 텍스트 영역 -->    
    			<textarea rows="10" cols="50" class="console" disabled="disabled">
    			</textarea>  
    			...
    			webSocket.onmessage = function(message) {           // 서버로 부터 메시지가 오면  
    				...
    				// 메시지의 status는 유저의 접속 형태이다.      // visit은 유저가 접속했을 때 알리는 메시지.      
    				if(node.status === "visit") {        // 위 템플릿 div를 취득.        
    					let form = $(".template").html();        // div를 감싸고 속성 data-key에 unique키 넣음.        
    					form = $("<div class='float-left'></div>").attr("data-key",node.key).append(form);         // body에 추가한다.        
    					$("body").append(form);     
